...
export function selectModules(targetSkills, allModules, { knownClusters = [], maxHours = 100, fastTrack = false } = {}) {
  ...
  for (const m of allModules) {
    ...
    if (fastTrack && m.level === 0) s *= 0.45; // experienced learners: bias away from beginner modules
...
export function buildWeekPlan(targetSkills, allModules, opts = {}) {
  const { knownClusters = [], hoursPerWeek = 10, roleName = "your target role",
          maxWeeks = 26, fastTrack = false, projectFirst = false } = opts;
  const asyncBudgetPerWeek = hoursPerWeek * 0.4;
  // reserve ~28% of the timeline for hackathon + capstone weeks; honor the learner's deadline
  const maxContentWeeks = Math.max(4, Math.min(34, Math.floor(maxWeeks * 0.72)));
  const maxAsync = asyncBudgetPerWeek * maxContentWeeks;
  const mods = selectModules(targetSkills, allModules, { knownClusters, maxHours: maxAsync, fastTrack });
