...
const SYNONYMS = {
  ...
  "recruitment": "Talent Acquisition", "talent acquisition": "Talent Acquisition", "hr ": "Human Resource Management",
...
export function extractSkills(text, taxonomy) {
  ...
  for (const [syn, canon] of Object.entries(SYNONYMS)) {
    // word-boundary match (substring matching caused "git" ⊂ "digital" false positives)
    const needle = syn.trim().replace(/\//g, " ");
    const re = new RegExp("(^|[^a-z0-9.+#])" + escapeRe(needle) + "($|[^a-z0-9.+#])");
    if (re.test(lower)) found.set(canon, (found.get(canon) || 0) + 2);
  }
...
// Detailed extraction with evidence + must-have classification (for the JD analyzer).
export function extractSkillsDetailed(text, taxonomy) {
  const lines = text.split(/\n+/);
  const names = extractSkills(text, taxonomy);
  const NICE = /\b(plus|preferred|nice to have|good to have|bonus|advantage|desirable)\b/i;
  return names.map((name) => {
    const nl = name.toLowerCase();
    const syn = Object.entries(
      // find a synonym that maps to this canonical name
      SYNONYMS
    ).find(([k, v]) => v === name);
    const needle = syn ? syn[0].trim() : nl;
    const line = lines.find((l) => l.toLowerCase().includes(needle) || l.toLowerCase().includes(nl)) || "";
    return { name, evidence: line.trim().slice(0, 140), must: line ? !NICE.test(line) : true };
  });
}
