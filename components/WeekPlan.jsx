...
function ModulePreview({ course, num }) {
  ...
  if (!items) return <p className="mt-2 animate-pulse text-xs text-ink-400">Loading curriculum…</p>;
  return (
    ...
    <div>
      ...
      <div className="mt-4 flex flex-wrap items-center gap-2 print:hidden">
        ...
        {editCount > 0 && (
          <span className="flex items-center gap-2 text-xs">
            ...
            <button onClick={resetEdits} className="text-ink-400 hover:text-brand-600">reset</button>
          </span>
        )}
        <span className="ml-auto text-xs text-ink-400">📅 Done by <span className="text-ink-700">{projected}</span></span>
      </div>
      ...
      <div className="card mt-4 p-4">
        <div className="flex h-3.5 w-full overflow-hidden rounded">
          ...
          <div className="bg-flame-500" style={{ width: `${plan.blend.masterclassCoaching}%` }} />
          <div className="bg-ink-400" style={{ width: `${plan.blend.assessment}%` }} />
        </div>
        <div className="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-[11px] text-ink-500">
          ...
          <span><span className="mr-1 inline-block h-2 w-2 rounded-full bg-flame-500" />Masterclass+Coach {plan.blend.masterclassCoaching}%</span>
          <span><span className="mr-1 inline-block h-2 w-2 rounded-full bg-ink-400" />Assessment {plan.blend.assessment}%</span>
        </div>
      </div>
            ...
            <div key={w.n} className={`card overflow-hidden ${done[w.n] ? "opacity-60" : ""}`}>
              ...
              <button onClick={() => setExpanded((x) => ({ ...x, [w.n]: !x[w.n] }))}
                ...
                <span onClick={(e) => toggleWeek(w.n, e)} title="Mark week done"
                  className={`grid h-8 w-8 shrink-0 cursor-pointer place-items-center rounded-full border text-xs font-bold transition ${done[w.n] ? "border-teal-500 bg-teal-600 text-white" : "border-ink-300 text-ink-700 hover:border-brand-500"}`}>
                  {done[w.n] ? "✓" : w.n}
                </span>
                ...
                <div className="hidden items-center gap-2 sm:flex">
                  ...
                  <span className="w-8 text-right text-[11px] text-ink-400">{pct}%</span>
                </div>
              ...
              {isOpen && (
                <div className="border-t border-ink-200 px-4 pb-4 pt-3">
                        ...
                        {(added[w.n] || []).map((a) => (
                          <li key={a.id} className="rounded-lg border border-dashed border-brand-200 bg-ink-50 px-3 py-2 text-sm">
                            <div className="flex flex-wrap items-center gap-x-2 gap-y-1">
                              ...
                              <span className="text-xs text-ink-400">{a.hours}h</span>
                          ...
                          return (
                            <li key={orig.id} className="rounded-lg bg-ink-50 px-3 py-2.5 text-sm">
                              <div className="flex flex-wrap items-center gap-x-2 gap-y-1">
                                ...
                                <span className="text-xs text-ink-400">{a.hours}h · {a.videos} videos</span>
                                <span className="ml-auto flex gap-2.5 text-[11px]">
                                  <button onClick={() => setPreview(preview === orig.id ? null : orig.id)} className="text-ink-500 hover:text-brand-600">{preview === orig.id ? "hide" : "▶ preview"}</button>
                                  {customize && (
                                    <>
                              ...
                              {customize && swapOpen === orig.id && (
                                <div className="mt-2 space-y-1 border-t border-ink-200 pt-2">
                                  {swaps[orig.id] && <button onClick={() => doSwap(orig.id, null)} className="block w-full rounded bg-ink-50 px-2 py-1.5 text-left text-xs text-ink-700 hover:bg-ink-100">↩ Restore: {orig.title}</button>}
                                  {(alts || []).filter((x) => x.id !== (swaps[orig.id]?.id)).map((alt) => {
                                    const shared = alt.skills.filter((s) => orig.skills.includes(s)).length;
                                    return (
                                      <button key={alt.id} onClick={() => doSwap(orig.id, alt)} className="block w-full rounded bg-ink-50 px-2 py-1.5 text-left text-xs text-ink-700 hover:bg-ink-100">
                                        ⇄ {alt.title} <span className="text-ink-400">· {alt.hours}h · {alt.course}</span> <span className="text-teal-600">{shared} shared skills</span>
                                      </button>
                                    );
                                  })}
                                  {alts && alts.length === 0 && <p className="text-xs text-ink-400">No equivalent module — this one&apos;s unique.</p>}
                                </div>
                              )}
                            </li>
                          );
                          ...
                          {addOpen === w.n ? (
                            <div className="rounded-lg border border-ink-300 bg-ink-50 p-2">
                              ...
                              <div className="mt-1 space-y-1">
                                {searchResults.map((m) => (
                                  <button key={m.id} onClick={() => addModule(w.n, m)} className="block w-full rounded bg-ink-50 px-2 py-1.5 text-left text-xs text-ink-700 hover:bg-ink-100">
                                    + {m.title} <span className="text-ink-400">({m.hours}h · {m.course})</span>
                                  </button>
                                ))}
                                {addQuery.length >= 3 && searchResults.length === 0 && <p className="px-2 py-1 text-xs text-ink-400">No modules match.</p>}
                              </div>
                              <button onClick={() => { setAddOpen(null); setAddQuery(""); }} className="mt-1 px-2 text-[11px] text-ink-400 hover:text-brand-600">close</button>
                            </div>
                          ) : (
                      ...
                      {w.sync.map((s, i) => (
                        <li key={i}>
                          <label className="flex items-center gap-2 text-sm text-ink-700">
                            ...
                            <span className={itemDone[`${w.n}-sync-${i}`] ? "line-through opacity-60" : ""}>{s.title} <span className="text-xs text-ink-400">({s.hours}h)</span></span>
                          </label>
                        </li>
                      ))}
                      {w.masterclass && <li className="text-sm text-flame-600">★ {w.masterclass.title} <span className="text-xs text-ink-400">({w.masterclass.hours}h)</span></li>}
                  ...
                  <div className="mt-3 rounded-lg border border-ink-200 bg-ink-50 px-3 py-2.5">
                    <p className="text-sm text-teal-700">📦 Submit by Sunday: <span className="text-ink-800">{w.deliverable}</span></p>
                    {effAsync.length > 0 && (
                      <p className="mt-1 text-[11px] text-ink-400">🎯 Unlocks: {[...new Set(effAsync.flatMap((a) => (swaps[a.id] || a).skills))].slice(0, 4).join(" · ")}</p>
                    )}
                  </div>
                  ...
                  {w.addons?.length > 0 && (
                    <div className="mt-2 flex flex-wrap gap-2">
                      {w.addons.map((a, i) => (
                        <button key={i} className="chip border border-dashed border-ink-300 bg-ink-50 text-ink-700 hover:border-brand-500" title="Career Launchpad — coming soon">
